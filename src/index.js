...
const processName = (node, isComputed, types) => {
    const name = types.isMemberExpression(node) ? `${node.object.name}.${node.property.name}` : node.name;
    ...
    parentPath.findParent((path) => {
        ...
        if (path.isFunctionDeclaration() && (isProgramScope(path) || isDirectlyExported(path))) {
            ...
            candidates = candidates.filter(
                (item) =>
                    !item.isVariableDeclarator() && !item.isAssignmentExpression() && !item.isFunctionDeclaration()
            );
            candidates.push(path);
            return true;
        }
        ...
        if (path.isConditionalExpression() && path.parentPath.isObjectProperty()) {
            return true;
        }
...
const getCallExpressionFunctionParamIndex = (parentsList, callerPath, callExpressionArguments) => {
    ...
    const closestChildPath =
        parentsList[parentsList.findIndex((item, index) => item.isCallExpression() && index < callerIndex) - 1];
        ...
        candidates.forEach((candidatePath) => {
            ...
            if (candidatePath.isExportDefaultDeclaration() && displayNamePlacements.length === 0) {
                if (types.isCallExpression(candidatePath.node.declaration)) {
                    ...
                    const paramIndex = getCallExpressionFunctionParamIndex(
                        parents,
                        candidatePath,
            ...
            if (candidatePath.isObjectProperty()) {
                ...
                const nameNodes = [objectNamePath.node].concat(
                    getObjectPropertyNameNodes(candidatePath, types).reverse()
                );
